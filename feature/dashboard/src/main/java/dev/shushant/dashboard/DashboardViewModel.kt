...
class DashboardViewModel @Inject constructor(
    ...
    private fun fetchCurrencies() {
        viewModelScope.launch {
            ...
            when {
                ...
                result.isSuccess -> setState { state ->
                    state.copy(
                        currencies = result.getOrNull(), isLoading = false, error = null
                    )
                }
            }
        }
    ...
    private fun fetchExchangeRates(baseCurrency: String, amount: BigDecimal) {
        viewModelScope.launch {
            setState { state -> state.copy(isLoading = true) }
            val result = currencyConverterUseCase.invoke(base = BASE_C,
                symbols = currentState.currencies?.items?.joinToString(separator = ",") { it.currencyCode }
                    ?: "")
            when {
                result.isFailure -> {
                    setState { state ->
    ...
    fun updateBaseCurrency(baseCurrency: String, amount: String) {
    ...
    private fun convertCurrency(baseCurrency: String, sourceAmount: BigDecimal) {
...
data class DashboardUIState(
    ...
    val error: String? = null, val currencies: Currencies? = null,
